...
// -------------------------------------------------------------------------
// Data Source Demarcation — Fastmarkets vs NBR Data vs others
// -------------------------------------------------------------------------
export function renderSourceBreakdown(containerId) {
  const el = document.getElementById(containerId);
  if (!el) return;
  const products = AppState.raw?.products?.products || [];
  const counts = {};
  products.forEach((p) => (counts[p.source] = (counts[p.source] || 0) + 1));
  const tone = { "Fastmarkets": "fastmarkets", "NBR Data": "nbr" };
  const items = Object.entries(counts).sort((a, b) => b[1] - a[1]);
  const total = products.length || 1;
  el.innerHTML = `
    <div class="source-breakdown-head">
      <div class="source-breakdown-title">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 7h16M7 12h10M10 17h4"/></svg>
        Data Source Demarcation
      </div>
      <span class="source-breakdown-sub">Exchange-assessed physical prices (Fastmarkets) vs. Bangladesh customs-import values (NBR Data)</span>
    </div>
    <div class="source-breakdown-items">
      ${items.map(([name, count]) => `
        <div class="source-breakdown-item">
          <span class="source-dot ${tone[name] || "other"}"></span>
          <span class="source-name">${name}</span>
          <span class="source-count">${count}</span>
          <span class="source-share">${Math.round((count / total) * 100)}%</span>
        </div>`).join("")}
    </div>`;
}
