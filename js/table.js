...
const COLUMNS = [
  ...
  { key: "source", label: "Data Source", sortable: true, sourceBadge: true },
];
...
function rowHtml(p) {
  return `<tr data-id="${p.id}">
    ${COLUMNS.map((c) => {
      ...
      if (c.sourceBadge) {
        return `<td><span class="source-badge source-${sourceClass(p.source)}">${p.source || "—"}</span></td>`;
      }
      if (c.change) {
        const v = p[c.key];
...
function sourceClass(s) {
  ...
  if (v.includes("fastmarket")) return "fastmarkets";
  if (v.includes("nbr")) return "nbr";
  return "other";
}
